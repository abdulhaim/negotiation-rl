...
class Reinforce(object):
    def __init__(self, dialog, ctx_gen, args, engine, corpus, logger=None):
        ...
        self.writer = SummaryWriter("experiment-baseline")

    def log_metrics(self, values, episode):
        for key,value in values.items():
            self.writer.add_scalar(tag=key, scalar_value=float(value.strip('%').strip('s')), global_step=episode)
    ...
    def run(self):
        ...
        for ctxs in self.ctx_gen.iter(self.args.nepoch):
            ...
            if self.args.sv_train_freq > 0 and n % self.args.sv_train_freq == 0:
                ...
                self.engine.train_batch(batch, self.writer, n)
            ...
            if n % 5 == 0:
                self.log_metrics(self.dialog.return_metrics(), n)
